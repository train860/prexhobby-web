...
export type TColumn<TData, TValue> = {
...
const DataTable = <TData extends {id:string}, TValue>(
  ...
  const table = useReactTable({
    ...
    onSortingChange: (updater) => {
      ...
      const sort = newSortingValue.map((s) => `${s.id}_${s.desc ? "desc" : "asc"}`).join(",")
      navigate({ sort })
    },
                        ...
                        <DropdownMenuContent align="end" className="w-[160px]">
                          {
                            selection.buttons.map((buttons, index) => {
                              return (
                                <>
                                  {index > 0 && <DropdownMenuSeparator key={`sep-${index}`} />}

                                  {buttons.map((button) => {
                                    return (
                                      <DropdownMenuItem key={button.key}>{button.title}</DropdownMenuItem>
                                    )
                                  })}
                                </>
                              )
  ...
  const renderToolbar = () => {
    ...
    if (toolbar === true) {
      return (
        <DataTableToolbar table={table} viewOptions={viewOptions} filters={filters} />
      )
    }
    return toolbar
  }
                ...
                table.getRowModel().rows.map((row) => (
                  ...
                  >
                    {row.getVisibleCells().map((cell) => (
                      <TableCell width={(cell.column.columnDef.meta as any)?.width} key={cell.id}>
                        {flexRender(
